...
VERSAO_AJUSTE_APEX_PRONTO_USO = "2026-06-27_REAL"
VERSAO_AJUSTE_RELATORIO_UNICO = "2026-06-27"
VERSAO_AJUSTE_RELATORIOS_01_A_19 = "2026-06-28"
...
# Permite executar somente um relatório, sem apagar os demais RELATORIO_01..RELATORIO_12 do .env.
# Aceita nome (RELATORIO_04), índice (4) ou saved_value APEX (96461147945312616).
RELATORIO_UNICO = (
    os.getenv("RELATORIO_UNICO", "")
    or os.getenv("EXECUTAR_SOMENTE_RELATORIO", "")
    or os.getenv("SOMENTE_RELATORIO", "")
).strip()

RELATORIO_UNICO_NOME = os.getenv("RELATORIO_UNICO_NOME", "").strip()
RELATORIO_UNICO_SAVED_VALUE = os.getenv("RELATORIO_UNICO_SAVED_VALUE", "").strip()
RELATORIO_UNICO_INDICE = os.getenv("RELATORIO_UNICO_INDICE", "").strip()
...
RESPONSAVEL_POR_RELATORIO_NOME = {
    ...
    "RELATORIO_13": "RenatoMezzalira",
    "RELATORIO_14": "RenatoMezzalira",
    "RELATORIO_15": "RenatoMezzalira",

    "RELATORIO_16": "AdrielSilva",
    "RELATORIO_17": "AdrielSilva",

    "RELATORIO_18": "DeiseRosa",
    "RELATORIO_19": "DeiseRosa",
}
...
RESPONSAVEL_POR_SAVED_VALUE = {
    ...
    "96836102112838540": "RenatoMezzalira",
    "96840562521859779": "RenatoMezzalira",
    "96838388851852063": "RenatoMezzalira",

    "96896222567721463": "AdrielSilva",
    "96911676179989837": "AdrielSilva",

    "96898268944724974": "DeiseRosa",
    "96913647909993268": "DeiseRosa",
}
...
def resolver_responsavel_relatorio(nome, saved_value):
    ...
    Aceita tanto nomes simples, como RELATORIO_14, quanto nomes descritivos,
    como RELATORIO_14_RenatoMezzalira. Também permite sobrescrever pelo .env:

    RELATORIO_14_RESPONSAVEL=RenatoMezzalira
    RESPONSAVEL_RELATORIO_14=RenatoMezzalira
    """
    nome_original = (nome or "").strip()
    nome_normalizado = re.sub(r"[^A-Z0-9_]+", "_", remover_acentos(nome_original).upper()).strip("_")
    saved_value = (saved_value or "").strip()

    chaves_nome = [nome_normalizado]

    match = re.search(r"RELATORIO[_ ]?(\d{1,3})", nome_normalizado)
    if match:
        numero = int(match.group(1))
        numero_2 = f"{numero:02d}"
        numero_3 = f"{numero:03d}"
        chaves_nome.extend([
            f"RELATORIO_{numero_2}",
            f"RELATORIO_{numero_3}",
            f"RELATORIO_{numero}",
        ])

        for chave_env in (
            f"RELATORIO_{numero_2}_RESPONSAVEL",
            f"RESPONSAVEL_RELATORIO_{numero_2}",
            f"RELATORIO_{numero}_RESPONSAVEL",
            f"RESPONSAVEL_RELATORIO_{numero}",
        ):
            valor_env = os.getenv(chave_env, "").strip()
            if valor_env:
                return valor_env

    for chave in chaves_nome:
        responsavel = RESPONSAVEL_POR_RELATORIO_NOME.get(chave)
        if responsavel:
            return responsavel

    return (
        RESPONSAVEL_POR_SAVED_VALUE.get(saved_value)
...
def aba_consolidado_responsavel(responsavel):
    ...
    chave = re.sub(r"[^A-Z0-9]", "", remover_acentos(responsavel or "").upper())
    ...
    if chave in {"RENATOMEZZALIRA", "RENATO"}:
        return "RENATOMEZZALIRA"

    if chave in {"ADRIELSILVA", "ADRIEL"}:
        return "ADRIELSILVA"

    if chave in {"DEISEROSA", "DEISE"}:
        return "DEISEROSA"

    return "SEM_RESPONSAVEL"


def registrar_arquivo_para_consolidacao(
    relatorio,
    caminho_arquivo,
    tipo_origem,
    dados_cenario=None,
    ...
    if aba_destino == "SEM_RESPONSAVEL":
        print(
            f"[CONSOLIDADO][AVISO] Responsável não mapeado para {caminho.name}. "
            "Arquivo entrará na aba SEM_RESPONSAVEL para não ser perdido."
        )

    identificador_cenario, nome_cenario = montar_metadados_cenario_para_arquivo(dados_cenario, origem_linha or tipo_origem)
    comando_cenario_execucoes = texto_util((dados_cenario or {}).get("comando_cenario_execucoes") or montar_comando_cenario_execucoes(identificador_cenario))
    ...
    ARQUIVOS_DOWNLOAD_EXECUCAO.append(
        {
            ...
            "aba": aba_destino,
            "tipo": tipo_origem,
            "status_edit_cenario": status_edit_cenario or ("COM_EDIT_DOWNLOAD" if tipo_origem == "download" else "SEM_EDIT_PLACEHOLDER"),
            "identificador_cenario": identificador_cenario,
            "comando_cenario_execucoes": comando_cenario_execucoes,
            "nome_cenario": nome_cenario,
            "origem_linha": origem_linha or "",
            "pagina": numero_pagina or "",
        }
    )
...
# ============================================================
# FILTRO OPCIONAL: EXECUTAR SOMENTE UM RELATÓRIO
# ============================================================
def _normalizar_chave_relatorio(valor):
    valor = "" if valor is None else str(valor)
    valor = remover_acentos(valor).upper() if "remover_acentos" in globals() else unicodedata.normalize("NFKD", valor).encode("ASCII", "ignore").decode("ASCII").upper()
    valor = re.sub(r"[^A-Z0-9]+", "", valor)
    return valor.strip()


def _montar_lista_relatorios_disponiveis(relatorios):
    linhas = []
    for idx, relatorio in enumerate(relatorios, start=1):
        linhas.append(
            f"{idx}. {relatorio.get('nome', '')} | VALUE={relatorio.get('saved_value', '')} | "
            f"Líder={relatorio.get('responsavel', '') or 'não definido'}"
        )
    return "\n".join(linhas)


def resolver_relatorios_para_processar(relatorios):
    """
    Retorna todos os relatórios ou somente um, conforme .env.

    Formas aceitas:
    RELATORIO_UNICO=RELATORIO_04
    RELATORIO_UNICO=4
    RELATORIO_UNICO=96461147945312616

    Alternativas explícitas:
    RELATORIO_UNICO_NOME=RELATORIO_04
    RELATORIO_UNICO_INDICE=4
    RELATORIO_UNICO_SAVED_VALUE=96461147945312616
    """
    relatorios = list(relatorios or [])

    if not relatorios:
        return []

    saved_value = (RELATORIO_UNICO_SAVED_VALUE or "").strip()
    nome = (RELATORIO_UNICO_NOME or "").strip()
    indice = (RELATORIO_UNICO_INDICE or "").strip()
    valor_generico = (RELATORIO_UNICO or "").strip()

    if not any([saved_value, nome, indice, valor_generico]):
        return relatorios

    # 1) saved_value explícito tem prioridade máxima.
    if saved_value:
        for relatorio in relatorios:
            if str(relatorio.get("saved_value", "")).strip() == saved_value:
                return [relatorio]
        raise Exception(
            f"RELATORIO_UNICO_SAVED_VALUE={saved_value!r} não encontrado.\n"
            f"Relatórios disponíveis:\n{_montar_lista_relatorios_disponiveis(relatorios)}"
        )

    # 2) nome explícito.
    if nome:
        chave_nome = _normalizar_chave_relatorio(nome)
        for relatorio in relatorios:
            if _normalizar_chave_relatorio(relatorio.get("nome", "")) == chave_nome:
                return [relatorio]
        raise Exception(
            f"RELATORIO_UNICO_NOME={nome!r} não encontrado.\n"
            f"Relatórios disponíveis:\n{_montar_lista_relatorios_disponiveis(relatorios)}"
        )

    # 3) índice explícito, considerando que o usuário informa 1 para o primeiro relatório.
    if indice:
        try:
            posicao = int(indice)
        except ValueError as e:
            raise Exception(f"RELATORIO_UNICO_INDICE inválido: {indice!r}. Informe um número, exemplo: 4") from e

        if posicao < 1 or posicao > len(relatorios):
            raise Exception(
                f"RELATORIO_UNICO_INDICE={posicao} fora do intervalo. "
                f"Use de 1 até {len(relatorios)}.\n"
                f"Relatórios disponíveis:\n{_montar_lista_relatorios_disponiveis(relatorios)}"
            )
        return [relatorios[posicao - 1]]

    # 4) variável genérica: tenta primeiro saved_value, depois nome, depois índice.
    valor = valor_generico

    for relatorio in relatorios:
        if str(relatorio.get("saved_value", "")).strip() == valor:
            return [relatorio]

    chave_valor = _normalizar_chave_relatorio(valor)
    for relatorio in relatorios:
        if _normalizar_chave_relatorio(relatorio.get("nome", "")) == chave_valor:
            return [relatorio]

    if valor.isdigit() and len(valor) <= 3:
        posicao = int(valor)
        if 1 <= posicao <= len(relatorios):
            return [relatorios[posicao - 1]]

    raise Exception(
        f"RELATORIO_UNICO={valor!r} não foi encontrado como nome, índice ou saved_value.\n"
        f"Relatórios disponíveis:\n{_montar_lista_relatorios_disponiveis(relatorios)}"
    )
...
COLUNA_STATUS_EDIT_CENARIO = "STATUS_EDIT_CENARIO"
COLUNA_IDENTIFICADOR_CENARIO = "IDENTIFICADOR_DE_CENARIO"
COLUNA_COMANDO_CENARIO_EXECUCOES = "COMANDO_CENARIO_EXECUCOES"
COLUNA_NOME_CENARIO = "NOME_CENARIO"
COLUNA_PAGINA_APEX = "PAGINA_APEX"
COLUNA_ORIGEM_LINHA = "ORIGEM_LINHA"
...
def extrair_identificador_do_texto(texto):
    ...
    padroes = [
        r"\b[A-Z0-9_]+(?:\.[A-Z0-9_]+){2,}\.V\d+\b",  # BELLO.PTM.001.V2, MAIS_FRANGO.CTL.059.V6
...
def montar_comando_cenario_execucoes(identificador):
    """Monta o comando Playwright de referência para o identificador do cenário."""
    identificador = texto_util(identificador)

    if not identificador:
        return ""

    identificador_js = identificador.replace('\\', '\\\\').replace('"', '\\"')
    return f'page.locator("iframe[title=\"Cenário e Execuções\"]").content_frame.get_by_text("{identificador_js}").click()'


def extrair_identificador_cenario_do_modal(frame_cenario):
    """
    Captura o identificador real dentro do modal Cenário e Execuções.

    Exemplo esperado no modal:
    BELLO.PTM.001.V2
    MAIS_FRANGO.CTL.059.V6
    """
    try:
        texto_modal = frame_cenario.locator("body").inner_text(timeout=4000)
    except Exception:
        try:
            texto_modal = frame_cenario.locator("html").inner_text(timeout=4000)
        except Exception as e:
            return "", f"não consegui ler texto do modal: {e}"

    texto_modal = texto_util(texto_modal)

    padroes = [
        r"\b[A-Z0-9_]+(?:\.[A-Z0-9_]+){2,}\.V\d+\b",
        r"\b[A-Z0-9_]+\.[A-Z0-9_]+\.\d{2,4}\.V\d+\b",
    ]

    for padrao in padroes:
        match = re.search(padrao, texto_modal, flags=re.IGNORECASE)
        if match:
            return texto_util(match.group(0)), "capturado_no_modal_cenario_execucoes"

    return "", "identificador padrão *.Vn não encontrado no modal"


def completar_identificador_cenario_do_modal(frame_cenario, dados_cenario, origem):
    """Atualiza dados_cenario com o identificador real do modal, quando encontrado."""
    dados_cenario = dict(dados_cenario or {})
    identificador_modal, diagnostico = extrair_identificador_cenario_do_modal(frame_cenario)

    if identificador_modal:
        dados_cenario["identificador_de_cenario"] = identificador_modal
        dados_cenario["identificador_modal"] = identificador_modal
        dados_cenario["identificador"] = identificador_modal
        dados_cenario["comando_cenario_execucoes"] = montar_comando_cenario_execucoes(identificador_modal)
        dados_cenario["diagnostico_identificador_modal"] = diagnostico
        print(f"[CENÁRIO] Identificador real capturado no modal: {identificador_modal}")
    else:
        identificador_atual = texto_util(dados_cenario.get("identificador") or origem)
        dados_cenario["identificador_de_cenario"] = identificador_atual
        dados_cenario["comando_cenario_execucoes"] = montar_comando_cenario_execucoes(identificador_atual)
        dados_cenario["diagnostico_identificador_modal"] = diagnostico
        print(f"[CENÁRIO][AVISO] Identificador real não encontrado no modal para {origem}. Usando fallback: {identificador_atual}. Detalhe: {diagnostico}")
...
def montar_metadados_cenario_para_arquivo(dados_cenario, origem):
    ...
    identificador = texto_util(
        dados_cenario.get("identificador_de_cenario")
        or dados_cenario.get("identificador_modal")
        or dados_cenario.get("codigo_cenario")
        or dados_cenario.get("identificador")
        or extrair_identificador_do_texto(nome)
        or origem
    )
...
def nome_curto_cenario_arquivo(dados_cenario, origem, limite=70):
    """Monta um pedaço seguro do nome do arquivo usando o cenário real, não só tr_child."""
    identificador, nome = montar_metadados_cenario_para_arquivo(dados_cenario, origem)

    chave_origem = normalizar_texto_busca(origem)
    chave_identificador = normalizar_texto_busca(identificador)

    if chave_identificador == chave_origem or identificador.lower().startswith(("tr_child_", "editar_indice_")):
        base = nome or identificador or origem
    else:
        base = f"{identificador}_{nome}" if nome and nome != identificador else identificador

    base = limpar_nome_arquivo(base)
    if len(base) > limite:
        base = base[:limite].rstrip("._-")

    return base or limpar_nome_arquivo(origem)
...
def garantir_linha_metadados_csv(linhas, valores_metadados):
    """
    Garante que arquivos sem dados, principalmente placeholders, tenham uma linha preenchida.

    Antes o placeholder ficava só com o cabeçalho:
    LIDER_CENARIO;STATUS_EDIT_CENARIO;...

    Agora, quando o arquivo tem somente cabeçalho, cria a linha 2 com os metadados do cenário.
    """
    if not linhas:
        linhas.append([])
    ...
    cabecalho = [texto_util(v) for v in linhas[0]]
    linha = ["" for _ in cabecalho]

    for indice, nome_coluna in enumerate(cabecalho):
        chave = normalizar_texto_busca(nome_coluna)
        if chave in valores_metadados:
            linha[indice] = valores_metadados[chave]

    # Só cria a linha se houver pelo menos algum metadado preenchido.
    if any(texto_util(v) for v in linha):
        linhas.append(linha)


def enriquecer_csv_com_cenario(caminho, identificador, nome, lider_cenario, status_edit_cenario, numero_pagina, origem_linha, comando_cenario_execucoes=""):
    ...
    # LIDER_CENARIO | STATUS_EDIT_CENARIO | IDENTIFICADOR_DE_CENARIO | COMANDO_CENARIO_EXECUCOES | NOME_CENARIO | PAGINA_APEX | ORIGEM_LINHA | demais colunas
    inserir_coluna_metadado_csv(linhas, COLUNA_ORIGEM_LINHA, origem_linha)
    inserir_coluna_metadado_csv(linhas, COLUNA_PAGINA_APEX, numero_pagina)
    inserir_coluna_metadado_csv(linhas, COLUNA_NOME_CENARIO, nome)
    inserir_coluna_metadado_csv(linhas, COLUNA_COMANDO_CENARIO_EXECUCOES, comando_cenario_execucoes)
    inserir_coluna_metadado_csv(linhas, COLUNA_IDENTIFICADOR_CENARIO, identificador)
    inserir_coluna_metadado_csv(linhas, COLUNA_STATUS_EDIT_CENARIO, status_edit_cenario)
    inserir_coluna_metadado_csv(linhas, COLUNA_LIDER_CENARIO, lider_cenario)

    valores_metadados = {
        normalizar_texto_busca(COLUNA_LIDER_CENARIO): lider_cenario,
        normalizar_texto_busca(COLUNA_STATUS_EDIT_CENARIO): status_edit_cenario,
        normalizar_texto_busca(COLUNA_IDENTIFICADOR_CENARIO): identificador,
        normalizar_texto_busca(COLUNA_COMANDO_CENARIO_EXECUCOES): comando_cenario_execucoes,
        normalizar_texto_busca(COLUNA_NOME_CENARIO): nome,
        normalizar_texto_busca(COLUNA_PAGINA_APEX): numero_pagina,
        normalizar_texto_busca(COLUNA_ORIGEM_LINHA): origem_linha,
    }
    garantir_linha_metadados_csv(linhas, valores_metadados)
...
def garantir_linha_metadados_xlsx(ws, valores_metadados):
    """Garante linha 2 preenchida quando a planilha/placeholder tem somente cabeçalho."""
    if ws.max_row > 1:
        possui_algum_dado = False
        for col in range(1, ws.max_column + 1):
            if texto_util(ws.cell(row=2, column=col).value):
                possui_algum_dado = True
                break
    ...
    for indice, nome_coluna in enumerate(cabecalho, start=1):
        chave = normalizar_texto_busca(nome_coluna)
        if chave in valores_metadados:
            ws.cell(row=2, column=indice).value = valores_metadados[chave]


def enriquecer_xlsx_com_cenario(caminho, identificador, nome, lider_cenario, status_edit_cenario, numero_pagina, origem_linha, comando_cenario_execucoes=""):
    try:
        from openpyxl import load_workbook
    except Exception as e:
    ...
    for ws in wb.worksheets:
        # Ordem final das colunas:
        # LIDER_CENARIO | STATUS_EDIT_CENARIO | IDENTIFICADOR_DE_CENARIO | COMANDO_CENARIO_EXECUCOES | NOME_CENARIO | PAGINA_APEX | ORIGEM_LINHA | demais colunas
        inserir_coluna_metadado_xlsx(ws, COLUNA_ORIGEM_LINHA, origem_linha)
        inserir_coluna_metadado_xlsx(ws, COLUNA_PAGINA_APEX, numero_pagina)
        inserir_coluna_metadado_xlsx(ws, COLUNA_NOME_CENARIO, nome)
        inserir_coluna_metadado_xlsx(ws, COLUNA_COMANDO_CENARIO_EXECUCOES, comando_cenario_execucoes)
        inserir_coluna_metadado_xlsx(ws, COLUNA_IDENTIFICADOR_CENARIO, identificador)
        inserir_coluna_metadado_xlsx(ws, COLUNA_STATUS_EDIT_CENARIO, status_edit_cenario)
        ...
        garantir_linha_metadados_xlsx(ws, valores_metadados)
...
def enriquecer_arquivo_download_com_cenario(
    caminho,
    dados_cenario,
    origem,
    relatorio=None,
    ...
    """
    Inclui metadados do cenário dentro do arquivo baixado/criado.

    Colunas principais:
    LIDER_CENARIO, STATUS_EDIT_CENARIO, IDENTIFICADOR_CENARIO, NOME_CENARIO,
    PAGINA_APEX e ORIGEM_LINHA.
    """
    ...
    identificador, nome = montar_metadados_cenario_para_arquivo(dados_cenario, origem)
    comando_cenario_execucoes = texto_util((dados_cenario or {}).get("comando_cenario_execucoes") or montar_comando_cenario_execucoes(identificador))
    lider_cenario = resolver_lider_cenario_para_arquivo(relatorio)
    status_edit_cenario = texto_util(status_edit_cenario or "STATUS_NAO_INFORMADO")
    numero_pagina = texto_util(numero_pagina)
    origem_linha = texto_util(origem)
    extensao = caminho.suffix.lower()

    try:
        if extensao in {".csv", ".txt"}:
            enriquecer_csv_com_cenario(caminho, identificador, nome, lider_cenario, status_edit_cenario, numero_pagina, origem_linha, comando_cenario_execucoes)
        elif extensao in {".xlsx", ".xlsm"}:
            enriquecer_xlsx_com_cenario(caminho, identificador, nome, lider_cenario, status_edit_cenario, numero_pagina, origem_linha, comando_cenario_execucoes)
        elif extensao == ".xls":
        ...
        print(
            ...
            f"{COLUNA_STATUS_EDIT_CENARIO}={status_edit_cenario} | "
            f"{COLUNA_IDENTIFICADOR_CENARIO}={identificador} | "
            f"{COLUNA_COMANDO_CENARIO_EXECUCOES}={comando_cenario_execucoes} | "
            f"{COLUNA_NOME_CENARIO}={nome} | "
            f"{COLUNA_PAGINA_APEX}={numero_pagina} | "
            f"{COLUNA_ORIGEM_LINHA}={origem_linha}"
        )

    except Exception as e:
        print(f"[CENÁRIO][ERRO] Não consegui gravar metadados do cenário no arquivo {caminho.name}: {e}")
...
def criar_download_nao_iniciado(page, origem, numero_pagina, relatorio, dados_cenario=None):
    ...
    nome_cenario_arquivo = nome_curto_cenario_arquivo(dados_cenario, origem)

    if responsavel_download:
        nome_base = (
            f"pagina_{numero_pagina}_{responsavel_download}_SEM_EDIT_"
            f"{limpar_nome_arquivo(origem)}_{nome_cenario_arquivo}_Cenario_e_Execucao_nao_iniciado"
        )
    else:
        nome_base = f"pagina_{numero_pagina}_SEM_EDIT_{limpar_nome_arquivo(origem)}_{nome_cenario_arquivo}_Cenario_e_Execucao_nao_iniciado"
    ...
    try:
        ...
        enriquecer_arquivo_download_com_cenario(
            destino,
            dados_cenario,
            origem,
            relatorio,
            status_edit_cenario="SEM_EDIT_PLACEHOLDER",
            numero_pagina=numero_pagina,
        )
        print(f"[PLACEHOLDER] Arquivo de cenário não iniciado criado: {destino}")
        registrar_arquivo_para_consolidacao(
            relatorio,
            destino,
            "placeholder_sem_edit",
...
def existe_overlay_apex_visivel(page):
    seletores = [
        ".u-Processing",
        ".a-Processing",
        ".apex_wait_overlay",
    ...
    for seletor in seletores:
        try:
            ...
            qtd = loc.count()
            for indice in range(min(qtd, 5)):
                try:
                    if loc.nth(indice).is_visible(timeout=100):
                        return True
                except Exception:
                    continue
        except Exception:
...
def detectar_condicao_tela_operavel(page):
    """Retorna qual condição mostra que o APEX está pronto para o robô agir."""
    candidatos = [
        ("CELL_EDITAR_VISIVEL", lambda: page.get_by_role("cell", name="Editar").first),
        ("LINKCOL_IRR_VISIVEL", lambda: page.locator("tr > .a-IRR-linkCol").first),
        ("BOTAO_PROXIMO_OU_IR_VISIVEL", lambda: page.get_by_role("button", name=re.compile(r"^(Ir|Próximo)$")).first),
    ]

    for nome, criar_locator in candidatos:
        try:
            loc = criar_locator()
            if loc.count() > 0 and loc.is_visible(timeout=300):
                try:
                    if loc.is_enabled(timeout=300):
                        return nome
                except Exception:
                    return nome
        except Exception:
...
def aguardar_apex_pronto_para_uso(
    page,
    relatorio=None,
    numero_pagina=None,
    origem="",
    etapa="APEX_PRONTO_PARA_USO",
    inicio_acao_perf=None,
    timeout_total_ms=None,
    registrar=True,
):
    """
    Mede o tempo REAL do APEX até a tela ficar operável.

    Tempo real termina quando o processamento APEX sumiu e há um elemento útil visível/clicável.
    Tempo bruto do robô fica em métricas separadas, pois pode incluir pequenas folgas de estabilidade.
    """
    if inicio_acao_perf is None:
        inicio_acao_perf = time.perf_counter()

    timeout_total_ms = timeout_total_ms or TIMEOUT_PADRAO
    limite_segundos = max(timeout_total_ms / 1000, 1)
    ...
    try:
        page.wait_for_load_state("domcontentloaded", timeout=min(timeout_total_ms, 20000))
        fase_dom = segundos_decorridos(inicio_acao_perf)
    except Exception as e:
        detalhes.append(f"domcontentloaded não confirmado: {e}")

    try:
        page.wait_for_load_state("networkidle", timeout=min(timeout_total_ms, 15000))
        fase_network = segundos_decorridos(inicio_acao_perf)
    except Exception as e:
        detalhes.append(f"networkidle não confirmado: {e}")

    while True:
        decorrido_total = time.perf_counter() - inicio_acao_perf
        if decorrido_total >= limite_segundos:
            status = "TIMEOUT_AGUARDANDO_APEX_OPERAVEL"
            break

        if not existe_overlay_apex_visivel(page):
            fase_overlay = segundos_decorridos(inicio_acao_perf)
            condicao = detectar_condicao_tela_operavel(page)
            if condicao:
                fase_operavel = segundos_decorridos(inicio_acao_perf)
                break

        time.sleep(0.25)

    duracao_total = segundos_decorridos(inicio_acao_perf)

    detalhe_final = (
        f"fase_domcontentloaded={fase_dom if fase_dom is not None else 'N/A'}s; "
        f"fase_networkidle={fase_network if fase_network is not None else 'N/A'}s; "
        f"fase_overlay_sumiu={fase_overlay if fase_overlay is not None else 'N/A'}s; "
        f"fase_tela_operavel={fase_operavel if fase_operavel is not None else 'N/A'}s; "
        f"condicao_operavel={condicao or 'N/A'}"
    )

    if detalhes:
        detalhe_final += " | " + " | ".join(str(d) for d in detalhes)
    ...
    return {
        ...
        "status": status,
        "duracao_segundos": duracao_total,
        "domcontentloaded_segundos": fase_dom,
        "networkidle_segundos": fase_network,
        "overlay_sumiu_segundos": fase_overlay,
        "tela_operavel_segundos": fase_operavel,
        "condicao_operavel": condicao,
        "detalhes": detalhe_final,
    }


def aguardar_execucao_testes_pronta_para_uso(page, inicio_acao_perf):
    """Mede quando a tela Execução de Testes fica pronta para selecionar relatório."""
    select_id = None
    try:
        if RELATORIOS:
            select_id = seletor_select_relatorio(RELATORIOS[0])
    except Exception:
        select_id = None
    ...
    try:
        if select_id:
            page.locator(select_id).wait_for(state="visible", timeout=TIMEOUT_PADRAO)
            condicao = f"select_relatorio_visivel={select_id}"
        else:
            page.get_by_role("button", name="Ir", exact=True).first.wait_for(state="visible", timeout=TIMEOUT_PADRAO)
            condicao = "botao_ir_visivel"
    except Exception as e:
        condicao = f"nao_confirmado: {e}"

    registrar_metrica_execucao(
        etapa="APEX_EXECUCAO_TESTES_PRONTA_USO",
        status="OK" if not condicao.startswith("nao_confirmado") else "AVISO",
        duracao_segundos=segundos_decorridos(inicio_acao_perf),
        detalhes=f"Tempo desde clicar em Execução de Testes até a tela ficar operável. {condicao}",
    )
...
def escrever_aba_resumo_execucao(
    ...
    metricas_login_bruto = [m for m in METRICAS_EXECUCAO if m.get("etapa") == "LOGIN_E_ABERTURA_EXECUCAO_TESTES_BRUTO_ROBO"]
    metricas_execucao_pronta = [m for m in METRICAS_EXECUCAO if m.get("etapa") == "APEX_EXECUCAO_TESTES_PRONTA_USO"]
    metricas_relatorio_real = [m for m in METRICAS_EXECUCAO if m.get("etapa") == "RELATORIO_APEX_PRONTO_USO_REAL"]
    metricas_relatorio_bruto = [m for m in METRICAS_EXECUCAO if m.get("etapa") == "CARGA_RELATORIO_APEX_BRUTO_ROBO"]
    metricas_proxima_real = [m for m in METRICAS_EXECUCAO if m.get("etapa") == "PROXIMA_PAGINA_APEX_PRONTA_USO_REAL"]
    metricas_proxima_bruto = [m for m in METRICAS_EXECUCAO if m.get("etapa") == "CARGA_PROXIMA_PAGINA_APEX_BRUTO_ROBO"]

    escrever_linha_resumo(ws, ["Tempo bruto login/abertura Execução de Testes - segundos", metricas_login_bruto[-1].get("duracao_segundos", 0) if metricas_login_bruto else 0])
    escrever_linha_resumo(ws, ["Tempo real Execução de Testes pronta para uso - segundos", metricas_execucao_pronta[-1].get("duracao_segundos", 0) if metricas_execucao_pronta else 0])
    escrever_linha_resumo(ws, ["Tempo médio REAL relatório APEX pronto para uso - segundos", duracao_media([m.get("duracao_segundos") for m in metricas_relatorio_real])])
    escrever_linha_resumo(ws, ["Tempo médio bruto carga relatório APEX/robô - segundos", duracao_media([m.get("duracao_segundos") for m in metricas_relatorio_bruto])])
    escrever_linha_resumo(ws, ["Tempo médio REAL próxima página APEX pronta para uso - segundos", duracao_media([m.get("duracao_segundos") for m in metricas_proxima_real])])
    escrever_linha_resumo(ws, ["Tempo médio bruto próxima página APEX/robô - segundos", duracao_media([m.get("duracao_segundos") for m in metricas_proxima_bruto])])
    ...
    for aba in ["WALCERI", "LUCASRAMOS", "CAMILA", "RENATOMEZZALIRA", "ADRIELSILVA", "DEISEROSA", "SEM_RESPONSAVEL"]:
    ...
    for info in estatisticas_arquivos:
        escrever_linha_resumo(
            ...
            [
                ...
                info.get("arquivo", ""),
                info.get("tipo", ""),
                info.get("status_edit_cenario", ""),
                info.get("identificador_cenario", ""),
                info.get("comando_cenario_execucoes", ""),
                info.get("nome_cenario", ""),
                info.get("pagina", ""),
                info.get("origem_linha", ""),
                info.get("status", ""),
...
def consolidar_arquivos_download_execucao(resumos_execucao=None, inicio_execucao=None):
    ...
    abas_obrigatorias = ["WALCERI", "LUCASRAMOS", "CAMILA", "RENATOMEZZALIRA", "ADRIELSILVA", "DEISEROSA", "SEM_RESPONSAVEL"]
...
def login_e_abrir_execucao_testes(page):
    ...
    inicio_execucao_testes_perf = time.perf_counter()
    item_execucao.click()

    aguardar_execucao_testes_pronta_para_uso(page, inicio_execucao_testes_perf)
    ...
    registrar_metrica_execucao(
        etapa="LOGIN_E_ABERTURA_EXECUCAO_TESTES_BRUTO_ROBO",
        status="OK",
        duracao_segundos=segundos_decorridos(inicio_login_perf),
        detalhes="Tempo bruto desde abrir URL de login até a tela de Execução de Testes ficar carregada. Inclui preenchimento, cliques e pequenas esperas controladas do robô.",
    )
...
def selecionar_relatorio_apex(page, relatorio):
    ...
    resultado_pronto = aguardar_apex_pronto_para_uso(
        page,
        relatorio=relatorio,
        origem="botao_ir_relatorio",
        etapa="RELATORIO_APEX_PRONTO_USO_REAL",
        inicio_acao_perf=inicio_carga_apex_perf,
        timeout_total_ms=TIMEOUT_PADRAO,
        registrar=True,
    )

    # Pequena folga de estabilidade depois da tela já estar operável.
    # Não entra na métrica REAL; entra apenas na métrica bruta do robô abaixo.
    page.wait_for_timeout(1000)

    tempo_carga_apex_bruto = segundos_decorridos(inicio_carga_apex_perf)
    registrar_metrica_execucao(
        etapa="CARGA_RELATORIO_APEX_BRUTO_ROBO",
        relatorio=relatorio,
        status=resultado_pronto.get("status", "OK"),
        duracao_segundos=tempo_carga_apex_bruto,
        detalhes=(
            f"Tempo bruto após clicar em Ir até seguir o fluxo. "
            f"Tempo real operável={resultado_pronto.get('duracao_segundos')}s. "
            f"Tempo total da seleção={segundos_decorridos(inicio_selecao_perf)}s. "
            f"{resultado_pronto.get('detalhes', '')}"
        ),
    )
...
def baixar_execucao_aberta(page, origem, numero_pagina, relatorio, dados_cenario=None, inicio_linha_perf=None):
    ...
    # Isso reduz falso "sem Edit" em telas lentas e permite capturar o identificador real do cenário.
    try:
    ...
    dados_cenario = completar_identificador_cenario_do_modal(frame_cenario, dados_cenario, origem)
    ...
    if not link_edit or qtd_edit <= 0:
        ...
        registrar_metrica_execucao(
            ...
            detalhes=f"Identificador cenário: {montar_metadados_cenario_para_arquivo(dados_cenario, origem)[0]}. Busca Edit: {tempo_busca_edit}s. Geração placeholder: {tempo_placeholder}s. Estratégia/diagnóstico: {estrategia_edit}.",
    ...
    if responsavel_download:
        nome_final = f"pagina_{numero_pagina}_{responsavel_download}_COM_EDIT_{limpar_nome_arquivo(origem)}_{nome_cenario_arquivo}_{nome_original}"
    else:
        nome_final = f"pagina_{numero_pagina}_COM_EDIT_{limpar_nome_arquivo(origem)}_{nome_cenario_arquivo}_{nome_original}"
    ...
    enriquecer_arquivo_download_com_cenario(
        ...
        origem,
        relatorio,
        status_edit_cenario="COM_EDIT_DOWNLOAD",
        numero_pagina=numero_pagina,
    )
    ...
    registrar_metrica_execucao(
        ...
        status="DOWNLOAD_OK",
        duracao_segundos=tempo_total_linha,
        detalhes=f"Identificador cenário: {montar_metadados_cenario_para_arquivo(dados_cenario, origem)[0]}. Busca Edit: {tempo_busca_edit}s. Download bruto: {tempo_download_bruto}s. Estratégia Edit: {estrategia_edit}. Links Edit encontrados: {qtd_edit}.",
        arquivo=destino,
    )
...
def processar_pagina_atual(page, numero_pagina, relatorio):
    ...
    pronto = aguardar_relatorio_pronto(page)
    registrar_metrica_execucao(
        etapa="PAGINA_ATUAL_CONFIRMADA_OPERAVEL",
        relatorio=relatorio,
        numero_pagina=numero_pagina,
        origem=f"pagina_{numero_pagina}",
        status="OK" if pronto else "AVISO_SEM_CONFIRMACAO",
        duracao_segundos=segundos_decorridos(inicio_pagina_pronta_perf),
        detalhes="Tempo gasto na confirmação de que a página atual possui linha/Edit disponível para iniciar a varredura.",
    )
...
def clicar_proximo_se_existir(page, numero_pagina, relatorio=None):
    ...
    for criar_locator in candidatos:
        try:
            ...
            print(f"[PÁGINA {numero_pagina}] Aguardando próxima página carregar até ficar operável...")
            resultado_pronto = aguardar_apex_pronto_para_uso(
                page,
                relatorio=relatorio,
                numero_pagina=numero_pagina + 1,
                origem=f"pagina_{numero_pagina}_para_{numero_pagina + 1}",
                etapa="PROXIMA_PAGINA_APEX_PRONTA_USO_REAL",
                inicio_acao_perf=inicio_proxima_pagina_perf,
                timeout_total_ms=TIMEOUT_PADRAO,
                registrar=True,
            )
            ...
            registrar_metrica_execucao(
                etapa="CARGA_PROXIMA_PAGINA_APEX_BRUTO_ROBO",
                relatorio=relatorio,
                numero_pagina=numero_pagina + 1,
                origem=f"pagina_{numero_pagina}_para_{numero_pagina + 1}",
                status=resultado_pronto.get("status", "OK"),
                ...
                detalhes=(
                    f"Tempo bruto após clicar em Próximo até seguir o fluxo. "
                    f"Tempo real operável={resultado_pronto.get('duracao_segundos')}s. "
                    f"{resultado_pronto.get('detalhes', '')}"
                ),
            )
...
def run():
    ...
    relatorios_processamento = resolver_relatorios_para_processar(RELATORIOS)

    print("[CONFIGURAÇÃO] Relatórios carregados do .env")
    print(f"[CONFIGURAÇÃO] Total de relatórios carregados: {len(RELATORIOS)}")
    print(f"[CONFIGURAÇÃO] Total de relatórios que serão processados nesta execução: {len(relatorios_processamento)}")

    filtro_relatorio_unico = RELATORIO_UNICO_SAVED_VALUE or RELATORIO_UNICO_NOME or RELATORIO_UNICO_INDICE or RELATORIO_UNICO
    if filtro_relatorio_unico:
        print(f"[CONFIGURAÇÃO] Filtro de relatório único ativo: {filtro_relatorio_unico}")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            for indice, relatorio in enumerate(relatorios_processamento, start=1):
                cronometro.atualizar_status(
                    f"Processando {relatorio['nome']} ({indice}/{len(relatorios_processamento)})"
                )

                print("=" * 100)
                print(f"[LOOP] Iniciando relatório {indice}/{len(relatorios_processamento)}: {relatorio['nome']}")
                print("=" * 100)
                ...
                try:
                    ...
                    if indice < len(relatorios_processamento):
                        proximo = relatorios_processamento[indice]
