# Script individual do RELATORIO_18_DeiseRosa_P1
# Padrao V7 resiliente: carrega credenciais/env_DEISEROSA_P1.env, normaliza o relatorio exclusivo, aplica metricas e otimizações seguras.
...
RELATORIO_INDICE_ORIGINAL = "18"
RELATORIO_INDICE_MOTOR = "1"
RELATORIO_NOME = "RELATORIO_18_DeiseRosa_P1"
RELATORIO_LIDER = "DeiseRosa"
RELATORIO_PRIORIDADE = "P1"
RELATORIO_TOTAL_CENARIOS = "111"
RELATORIO_EDITAR_INICIO = "0"
RELATORIO_EDITAR_FIM = "5"
RELATORIO_TR_INICIO = "9"
RELATORIO_TR_FIM = "113"
...
RELATORIO_SAVED_VALUE = "96898268944724974"
ARQUIVO_ENV = "env_DEISEROSA_P1.env"
...
def _intervalo_config(nome: str, padrao: str) -> str:
    """Permite retomada resiliente por variáveis GTN_RESUME_* sem alterar o script."""
    valor = os.environ.get(f"GTN_RESUME_{nome}")
    if valor is None or str(valor).strip() == "":
        valor = os.environ.get(nome, padrao)
    return str(valor).strip()


def obter_intervalo_relatorio() -> tuple[str, str, str, str]:
    """Retorna editar_inicio, editar_fim, tr_inicio e tr_fim efetivos."""
    editar_inicio = _intervalo_config("EDITAR_INICIO", RELATORIO_EDITAR_INICIO)
    editar_fim = _intervalo_config("EDITAR_FIM", RELATORIO_EDITAR_FIM)
    tr_inicio = _intervalo_config("TR_INICIO", RELATORIO_TR_INICIO)
    tr_fim = _intervalo_config("TR_FIM", RELATORIO_TR_FIM)
    return editar_inicio, editar_fim, tr_inicio, tr_fim
...
def montar_linha_relatorio() -> str:
    ...
    editar_inicio, editar_fim, tr_inicio, tr_fim = obter_intervalo_relatorio()
    return (
        f"{RELATORIO_NOME}|{RELATORIO_APEX_ID}|{RELATORIO_SAVED_VALUE}|"
        f"{editar_inicio}|{editar_fim}|{tr_inicio}|{tr_fim};"
    )
...
def preparar_execucao_exclusiva() -> None:
    ...
    editar_inicio, editar_fim, tr_inicio, tr_fim = obter_intervalo_relatorio()
    os.environ["RELATORIO_ATUAL_EDITAR_INICIO"] = editar_inicio
    os.environ["RELATORIO_ATUAL_EDITAR_FIM"] = editar_fim
    os.environ["RELATORIO_ATUAL_TR_INICIO"] = tr_inicio
    os.environ["RELATORIO_ATUAL_TR_FIM"] = tr_fim
