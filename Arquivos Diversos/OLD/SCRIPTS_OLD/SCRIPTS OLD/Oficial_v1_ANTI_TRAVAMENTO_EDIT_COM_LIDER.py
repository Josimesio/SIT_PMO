...
VERSAO_AJUSTE_ANTI_TRAVAMENTO_EDIT = "2026-06-27"
...
TIMEOUT_EDIT_EXECUCAO = env_int("TIMEOUT_EDIT_EXECUCAO", "5000")
# Limite real de espera por Edit. Evita que cenário sem execução deixe o robô parado tempo demais.
TIMEOUT_EDIT_EXECUCAO_MAXIMO_REAL = env_int("TIMEOUT_EDIT_EXECUCAO_MAXIMO_REAL", "5000")
INTERVALO_LOG_EDIT_SEGUNDOS = env_int("INTERVALO_LOG_EDIT_SEGUNDOS", "2")
...
def localizar_link_edit(frame_cenario, origem=""):
    """
    Localiza um link Edit visível sem deixar o processo travado.

    Mudança importante desta versão:
    - usa busca rápida via DOM/JavaScript dentro do iframe;
    - limita a espera real pelo Edit;
    - se não achar, retorna None para criar placeholder e seguir a fila.
    """
    limite_config_ms = max(int(TIMEOUT_EDIT_EXECUCAO or 5000), 1000)
    limite_maximo_ms = max(int(TIMEOUT_EDIT_EXECUCAO_MAXIMO_REAL or 5000), 1000)
    limite_ms = min(limite_config_ms, limite_maximo_ms)
    limite_segundos = max(limite_ms / 1000, 1)

    inicio = time.perf_counter()
    proximo_log = max(int(INTERVALO_LOG_EDIT_SEGUNDOS or 2), 1)
    ultimo_erro = None
    ...
    while True:
        ...
        if decorrido >= limite_segundos:
            detalhe = f"timeout rápido de {limite_ms}ms atingido"
            if ultimo_erro:
        ...
        if decorrido >= proximo_log:
            print(
                f"[AGUARDANDO EDIT] {origem}: procurando Edit "
                f"({int(decorrido)}s/{int(limite_segundos)}s). Se não achar, segue com placeholder..."
            )
            proximo_log += max(int(INTERVALO_LOG_EDIT_SEGUNDOS or 2), 1)

        try:
            resultado = frame_cenario.evaluate(script_busca_edit)
            qtd = int((resultado or {}).get("quantidade") or 0)
            indice = int((resultado or {}).get("primeiroIndice") or -1)

            if qtd > 0 and indice >= 0:
                return frame_cenario.locator("a").nth(indice), qtd, "busca DOM rápida por link Edit"

        except Exception as e:
            ultimo_erro = e

            # Fallback simples por CSS, também rápido.
            try:
                locator = frame_cenario.locator("a").filter(has_text=re.compile(r"^\s*Edit\s*$", re.I))
                qtd = locator.count()
                if qtd > 0 and locator.first.is_visible(timeout=250):
                    return locator.first, qtd, "fallback CSS rápido por texto Edit"
...
def baixar_execucao_aberta(page, origem, numero_pagina, relatorio, dados_cenario=None):
    ...
    if not link_edit or qtd_edit <= 0:
        print(f"[SEM EDIT] {origem} não possui link Edit visível dentro do limite rápido configurado.")
