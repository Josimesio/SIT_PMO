...
TIMEOUT_EDIT_EXECUCAO = env_int("TIMEOUT_EDIT_EXECUCAO", "20000")
INTERVALO_LOG_EDIT_SEGUNDOS = env_int("INTERVALO_LOG_EDIT_SEGUNDOS", "5")
MAX_LINHAS_INEXISTENTES_SEGUIDAS = env_int("MAX_LINHAS_INEXISTENTES_SEGUIDAS", "10")

# ============================================================
# MODO TESTE - UMA LINHA ESPECÍFICA
# ============================================================
# Esta versão foi preparada para validar apenas uma linha antes de rodar a carga completa.
#
# Como ajustar no .env:
# MODO_TESTE_LINHA_ESPECIFICA=S
#
# Você pode escolher o relatório de 3 formas:
# 1) Por índice carregado do .env:
#    TESTE_RELATORIO_INDICE=1
#
# 2) Por nome exato ou trecho do nome:
#    TESTE_RELATORIO_NOME=RELATORIO_05
#
# 3) Pelo saved_value real do APEX:
#    TESTE_RELATORIO_SAVED_VALUE=96466495992333195
#
# Se TESTE_RELATORIO_SAVED_VALUE ou TESTE_RELATORIO_NOME forem informados, eles têm prioridade sobre o índice.
#
# TESTE_TIPO_LINHA=TR
# TESTE_LINHA_VALOR=9
# TESTE_PAGINA_ALVO=1
#
# TESTE_TIPO_LINHA:
# - TR      => usa tr:nth-child(TESTE_LINHA_VALOR) > .a-IRR-linkCol
# - EDITAR  => usa page.get_by_role("cell", name="Editar").nth(TESTE_LINHA_VALOR)
#
# Exemplo para testar o tr:nth-child(22):
# TESTE_TIPO_LINHA=TR
# TESTE_LINHA_VALOR=22
#
# Exemplo para testar o primeiro "Editar":
# TESTE_TIPO_LINHA=EDITAR
# TESTE_LINHA_VALOR=0
MODO_TESTE_LINHA_ESPECIFICA = os.getenv("MODO_TESTE_LINHA_ESPECIFICA", "S").strip().upper() in {"S", "SIM", "TRUE", "1", "YES"}
TESTE_RELATORIO_INDICE = env_int("TESTE_RELATORIO_INDICE", "1")
TESTE_RELATORIO_NOME = os.getenv("TESTE_RELATORIO_NOME", "").strip()
TESTE_RELATORIO_SAVED_VALUE = (
    os.getenv("TESTE_RELATORIO_SAVED_VALUE", "").strip()
    or os.getenv("TESTE_RELATORIO_VALUE", "").strip()
)
TESTE_TIPO_LINHA = os.getenv("TESTE_TIPO_LINHA", "TR").strip().upper()
TESTE_LINHA_VALOR = env_int("TESTE_LINHA_VALOR", "9")
TESTE_PAGINA_ALVO = env_int("TESTE_PAGINA_ALVO", "1")

# Modo teste com várias linhas e vários relatórios.
# Formato recomendado:
# TESTE_LINHAS_ESPECIFICAS=RELATORIO_01:TR=14,15;RELATORIO_04:TR=9,19,30,150
# Também aceita EDITAR, por exemplo:
# TESTE_LINHAS_ESPECIFICAS=RELATORIO_01:EDITAR=0,1
TESTE_LINHAS_ESPECIFICAS = os.getenv("TESTE_LINHAS_ESPECIFICAS", "").strip()
...
def texto_util(valor):
    ...
    Esta função fica aqui propositalmente porque o modo teste é resolvido
    antes do bloco de utilitários mais abaixo do arquivo.
    Sem isso, Python quebra com NameError ao ler TESTE_LINHAS_ESPECIFICAS.
    """
    valor = "" if valor is None else str(valor)
    return re.sub(r"\s+", " ", valor).strip()


def normalizar_chave_teste(valor):
    valor = "" if valor is None else str(valor)
...
def resolver_relatorio_teste(relatorios):
    """Escolhe o relatório do modo teste por saved_value, nome ou índice."""
    if not relatorios:
        raise Exception("Modo teste ativo, mas nenhum relatório foi carregado do .env.")

    saved_value_alvo = (TESTE_RELATORIO_SAVED_VALUE or "").strip()

    if saved_value_alvo:
        for posicao, relatorio in enumerate(relatorios, start=1):
            if str(relatorio.get("saved_value", "")).strip() == saved_value_alvo:
                return dict(relatorio), f"saved_value={saved_value_alvo} (posição {posicao})"

        print(
            f"[MODO TESTE][AVISO] Não encontrei relatório com "
            f"TESTE_RELATORIO_SAVED_VALUE={saved_value_alvo}. Vou tentar por nome/índice."
        )

    nome_alvo = normalizar_chave_teste(TESTE_RELATORIO_NOME)

    if nome_alvo:
        # Primeiro tenta igualdade exata. Depois aceita trecho do nome, para facilitar testes rápidos.
        for posicao, relatorio in enumerate(relatorios, start=1):
            nome_relatorio = normalizar_chave_teste(relatorio.get("nome", ""))
            if nome_relatorio == nome_alvo:
                return dict(relatorio), f"nome={relatorio.get('nome', '')} (posição {posicao})"

        for posicao, relatorio in enumerate(relatorios, start=1):
            nome_relatorio = normalizar_chave_teste(relatorio.get("nome", ""))
            if nome_alvo in nome_relatorio:
                return dict(relatorio), f"nome contendo {TESTE_RELATORIO_NOME} (posição {posicao})"

        print(
            f"[MODO TESTE][AVISO] Não encontrei relatório com "
            f"TESTE_RELATORIO_NOME={TESTE_RELATORIO_NOME!r}. Vou usar o índice."
        )

    indice_relatorio_teste = max(TESTE_RELATORIO_INDICE, 1)

    if indice_relatorio_teste > len(relatorios):
        print(
            f"[MODO TESTE][AVISO] TESTE_RELATORIO_INDICE={TESTE_RELATORIO_INDICE} "
            f"maior que total de relatórios ({len(relatorios)}). Usando o primeiro relatório."
        )
        indice_relatorio_teste = 1

    return dict(relatorios[indice_relatorio_teste - 1]), f"índice={indice_relatorio_teste}"


def normalizar_tipo_linha_teste(tipo):
    tipo = normalizar_chave_teste(tipo)

    if tipo in {"EDITAR", "INDICE", "INDEX"}:
        return "EDITAR"

    return "TR"


def resolver_relatorio_teste_por_chave(relatorios, chave_relatorio):
    """Resolve relatório por saved_value, nome ou posição numérica."""
    chave_original = texto_util(chave_relatorio)

    if not chave_original:
        raise Exception("Chave de relatório vazia em TESTE_LINHAS_ESPECIFICAS.")

    # 1) Se for número pequeno, tenta como posição do relatório no .env.
    if re.fullmatch(r"\d+", chave_original):
        indice = int(chave_original)
        if 1 <= indice <= len(relatorios):
            return dict(relatorios[indice - 1]), f"índice={indice}"

    # 2) Tenta saved_value exato.
    for posicao, relatorio in enumerate(relatorios, start=1):
        if str(relatorio.get("saved_value", "")).strip() == chave_original:
            return dict(relatorio), f"saved_value={chave_original} (posição {posicao})"

    # 3) Tenta nome exato e depois trecho do nome.
    chave_norm = normalizar_chave_teste(chave_original)

    for posicao, relatorio in enumerate(relatorios, start=1):
        nome_relatorio = normalizar_chave_teste(relatorio.get("nome", ""))
        if nome_relatorio == chave_norm:
            return dict(relatorio), f"nome={relatorio.get('nome', '')} (posição {posicao})"

    for posicao, relatorio in enumerate(relatorios, start=1):
        nome_relatorio = normalizar_chave_teste(relatorio.get("nome", ""))
        if chave_norm in nome_relatorio:
            return dict(relatorio), f"nome contendo {chave_original} (posição {posicao})"

    raise Exception(
        f"Não encontrei relatório para a chave {chave_original!r} em TESTE_LINHAS_ESPECIFICAS. "
        "Use o nome do relatório, a posição no .env ou o saved_value do APEX."
    )


def parsear_linhas_teste_multiplas(valor_config):
    """
    Lê TESTE_LINHAS_ESPECIFICAS.

    Formato:
    RELATORIO_01:TR=14,15;RELATORIO_04:TR=9,19,30,150;RELATORIO_08:TR=9,12

    Cada bloco tem:
    <relatorio>:<tipo>=<linhas>
    - relatorio: nome, índice ou saved_value
    - tipo: TR ou EDITAR
    - linhas: lista separada por vírgula
    """
    valor_config = texto_util(valor_config)
    ...
    for bloco in [parte.strip() for parte in valor_config.split(";") if parte.strip()]:
        if ":" not in bloco or "=" not in bloco:
            raise Exception(
                f"Formato inválido em TESTE_LINHAS_ESPECIFICAS no trecho {bloco!r}. "
                "Use: RELATORIO_01:TR=14,15;RELATORIO_04:TR=9,19,30"
            )

        chave_relatorio, restante = bloco.split(":", 1)
        tipo_linha, linhas_raw = restante.split("=", 1)
        tipo_linha = normalizar_tipo_linha_teste(tipo_linha)

        linhas = []
        for item_linha in [item.strip() for item in linhas_raw.split(",") if item.strip()]:
            try:
                linhas.append(int(item_linha))
            except ValueError as e:
                raise Exception(
                    f"Linha inválida em TESTE_LINHAS_ESPECIFICAS: {item_linha!r}. "
                    "Informe apenas números separados por vírgula."
                ) from e

        if not linhas:
            raise Exception(f"Nenhuma linha informada no trecho {bloco!r}.")

        alvos.append(
            {
                "chave_relatorio": chave_relatorio.strip(),
                "tipo": tipo_linha,
                "linhas": linhas,
            }
        )
...
def resolver_relatorios_teste_multiplos(relatorios):
    """Monta uma lista de relatórios de teste com linhas explícitas, sem varrer intervalos."""
    alvos = parsear_linhas_teste_multiplas(TESTE_LINHAS_ESPECIFICAS)
    ...
    for alvo in alvos:
        relatorio, criterio = resolver_relatorio_teste_por_chave(relatorios, alvo["chave_relatorio"])
        chave_unica = relatorio.get("saved_value") or relatorio.get("nome")

        if chave_unica not in agrupados:
            relatorio["editar_inicio"] = -1
            relatorio["editar_fim"] = -1
            relatorio["tr_inicio"] = -1
            relatorio["tr_fim"] = -1
            relatorio["_teste_linhas_especificas"] = []
            relatorio["_teste_criterio_relatorio"] = criterio
            agrupados[chave_unica] = relatorio

        for linha in alvo["linhas"]:
            agrupados[chave_unica]["_teste_linhas_especificas"].append(
                {
                    "tipo": alvo["tipo"],
                    "linha": linha,
                    "origem_config": alvo["chave_relatorio"],
                }
            )
    ...
    for relatorio in resultado:
        ...
        vistos = set()
        linhas_unicas = []
        for alvo in relatorio.get("_teste_linhas_especificas", []):
            chave = (alvo.get("tipo"), alvo.get("linha"))
            if chave in vistos:
                continue
            vistos.add(chave)
            linhas_unicas.append(alvo)
        relatorio["_teste_linhas_especificas"] = linhas_unicas

    return resultado


# No modo teste, reduz a configuração para o(s) relatório(s) e linha(s) informados.
# Isso evita varrer milhares de cenários enquanto você valida o fluxo.
if MODO_TESTE_LINHA_ESPECIFICA:
    relatorios_teste_multiplos = resolver_relatorios_teste_multiplos(RELATORIOS)

    if relatorios_teste_multiplos:
        RELATORIOS = relatorios_teste_multiplos

        print("[MODO TESTE] Configuração múltipla ativa via TESTE_LINHAS_ESPECIFICAS.")
        for relatorio_teste in RELATORIOS:
            linhas_desc = ", ".join(
                f"{alvo['tipo']}={alvo['linha']}"
                for alvo in relatorio_teste.get("_teste_linhas_especificas", [])
            )
            print(
                f"[MODO TESTE] {relatorio_teste.get('nome')} | "
                f"VALUE={relatorio_teste.get('saved_value')} | "
                f"critério={relatorio_teste.get('_teste_criterio_relatorio', '')} | "
                f"linhas={linhas_desc}"
            )

    else:
        relatorio_teste, criterio_relatorio_teste = resolver_relatorio_teste(RELATORIOS)

        print(
            f"[MODO TESTE] Relatório escolhido por {criterio_relatorio_teste}: "
            f"{relatorio_teste.get('nome')} | VALUE={relatorio_teste.get('saved_value')}"
        )

        if TESTE_TIPO_LINHA in {"EDITAR", "INDICE", "INDEX"}:
            relatorio_teste["editar_inicio"] = TESTE_LINHA_VALOR
            relatorio_teste["editar_fim"] = TESTE_LINHA_VALOR
            relatorio_teste["tr_inicio"] = -1
            relatorio_teste["tr_fim"] = -1
        else:
            relatorio_teste["editar_inicio"] = -1
            relatorio_teste["editar_fim"] = -1
            relatorio_teste["tr_inicio"] = TESTE_LINHA_VALOR
            relatorio_teste["tr_fim"] = TESTE_LINHA_VALOR
...
def enriquecer_csv_com_cenario(caminho, identificador, nome):
    ...
    if not linhas:
        linhas = [[COLUNA_IDENTIFICADOR_CENARIO, COLUNA_NOME_CENARIO]]
    else:
        cabecalho = [texto_util(valor) for valor in linhas[0]]
        cabecalho_normalizado = [normalizar_texto_busca(valor) for valor in cabecalho]

        tem_identificador = normalizar_texto_busca(COLUNA_IDENTIFICADOR_CENARIO) in cabecalho_normalizado
        tem_nome = normalizar_texto_busca(COLUNA_NOME_CENARIO) in cabecalho_normalizado

        if not tem_nome:
            linhas[0].insert(0, COLUNA_NOME_CENARIO)
            for indice in range(1, len(linhas)):
                linhas[indice].insert(0, nome)

        if not tem_identificador:
            linhas[0].insert(0, COLUNA_IDENTIFICADOR_CENARIO)
            for indice in range(1, len(linhas)):
                linhas[indice].insert(0, identificador)

        # Se o arquivo só tinha cabeçalho, pelo menos as colunas novas ficam gravadas.
...
def enriquecer_xlsx_com_cenario(caminho, identificador, nome):
    try:
        from openpyxl import load_workbook
    except Exception as e:
    ...
    for ws in wb.worksheets:
        ...
        if normalizar_texto_busca(COLUNA_NOME_CENARIO) not in cabecalho_normalizado:
            ws.insert_cols(1)
            ws.cell(row=1, column=1).value = COLUNA_NOME_CENARIO
        ...
        if normalizar_texto_busca(COLUNA_IDENTIFICADOR_CENARIO) not in cabecalho_normalizado:
            ws.insert_cols(1)
            ws.cell(row=1, column=1).value = COLUNA_IDENTIFICADOR_CENARIO
            for row in range(2, ws.max_row + 1):
                ws.cell(row=row, column=1).value = identificador

    wb.save(caminho)


def enriquecer_arquivo_download_com_cenario(caminho, dados_cenario, origem):
    """
    Inclui IDENTIFICADOR_CENARIO e NOME_CENARIO dentro do arquivo baixado/criado.

    Para CSV/TXT: regrava o arquivo com duas colunas no início.
    Para XLSX/XLSM: insere duas colunas no início da primeira linha e preenche as linhas de dados.
    """
    ...
    try:
        if extensao in {".csv", ".txt"}:
            enriquecer_csv_com_cenario(caminho, identificador, nome)
        elif extensao in {".xlsx", ".xlsm"}:
            enriquecer_xlsx_com_cenario(caminho, identificador, nome)
        elif extensao == ".xls":
        ...
        print(
            ...
            f"{COLUNA_IDENTIFICADOR_CENARIO}={identificador} | {COLUNA_NOME_CENARIO}={nome}"
        )

    except Exception as e:
        print(f"[CENÁRIO][ERRO] Não consegui gravar nome/identificador no arquivo {caminho.name}: {e}")
...
def criar_download_nao_iniciado(page, origem, numero_pagina, relatorio, dados_cenario=None):
    ...
    try:
        ...
        enriquecer_arquivo_download_com_cenario(destino, dados_cenario, origem)
...
def localizar_link_edit(frame_cenario, origem=""):
    ...
    Importante:
    A versão anterior testava vários seletores em sequência e cada seletor
    podia esperar TIMEOUT_EDIT_EXECUCAO inteiro. Na prática, 4 seletores x
    20 segundos = até 80 segundos parado apenas em um cenário.

    Esta versão usa uma janela total de espera. Enquanto o tempo não estoura,
    ela procura por qualquer Edit visível de várias formas, sem ficar presa
    ...
    limite_segundos = max(TIMEOUT_EDIT_EXECUCAO / 1000, 1)
    proximo_log = INTERVALO_LOG_EDIT_SEGUNDOS
    ultimo_erro = None
    ultima_estrategia = "nenhuma"

    while True:
        decorrido = time.perf_counter() - inicio

        if decorrido >= limite_segundos:
            detalhe = f"timeout total de {TIMEOUT_EDIT_EXECUCAO}ms atingido"
            if ultimo_erro:
...
def baixar_execucao_aberta(page, origem, numero_pagina, relatorio, dados_cenario=None):
    ...
    if not link_edit or qtd_edit <= 0:
        ...
        criar_download_nao_iniciado(page, origem, numero_pagina, relatorio, dados_cenario)
...
def baixar_execucao_por_indice(page, indice, numero_pagina, relatorio):
    ...
    try:
        ...
        return baixar_execucao_aberta(page, origem, numero_pagina, relatorio, dados_cenario)
...
def processar_pagina_atual(page, numero_pagina, relatorio):
    ...
    alvos_teste = relatorio.get("_teste_linhas_especificas") or []

    if alvos_teste:
        print(f"[PÁGINA {numero_pagina}] Modo teste múltiplo: processando somente linhas explícitas.")

        for alvo in alvos_teste:
            tipo = normalizar_tipo_linha_teste(alvo.get("tipo"))
            linha = int(alvo.get("linha"))

            print(f"[PÁGINA {numero_pagina}] Alvo configurado: {tipo}={linha}")

            if tipo == "EDITAR":
                resultado = baixar_execucao_por_indice(page, linha, numero_pagina, relatorio)
            else:
                resultado = baixar_execucao_por_tr_child(page, linha, numero_pagina, relatorio)
        ...
        print(
            f"[PÁGINA {numero_pagina}] Modo teste múltiplo finalizado. "
...
def processar_todas_as_paginas(page, relatorio):
    ...
    if MODO_TESTE_LINHA_ESPECIFICA:
        pagina_alvo = max(TESTE_PAGINA_ALVO, 1)

        print("=" * 100)
        alvos_teste = relatorio.get("_teste_linhas_especificas") or []
        print("[MODO TESTE] Execução limitada à(s) linha(s) configurada(s).")
        print(f"[MODO TESTE] Relatório: {relatorio['nome']}")
        print(f"[MODO TESTE] Página alvo: {pagina_alvo}")
        if alvos_teste:
            linhas_desc = ", ".join(f"{alvo['tipo']}={alvo['linha']}" for alvo in alvos_teste)
            print(f"[MODO TESTE] Linhas configuradas: {linhas_desc}")
        else:
            print(f"[MODO TESTE] Tipo de linha: {TESTE_TIPO_LINHA}")
            print(f"[MODO TESTE] Valor da linha: {TESTE_LINHA_VALOR}")
        print("=" * 100)

        while numero_pagina < pagina_alvo:
            conseguiu_ir_proxima = clicar_proximo_se_existir(page, numero_pagina)

            if not conseguiu_ir_proxima:
                print(
                    f"[MODO TESTE][AVISO] Não consegui chegar na página alvo {pagina_alvo}. "
                    f"Vou testar na página atual ({numero_pagina})."
                )
                break
        ...
        print("=" * 100)
        print(f"[RESUMO DO TESTE] {relatorio['nome']}")
        print(f"Página testada: {numero_pagina}")
        print(f"Downloads OK: {total_downloads_ok}")
        print(f"Sem Edit/erro/pulados: {total_sem_edit_ou_erro}")
...
def run():
    ...
    if MODO_TESTE_LINHA_ESPECIFICA:
        print("[MODO TESTE] Ativo: o script vai processar somente linha(s) específica(s).")
        if TESTE_LINHAS_ESPECIFICAS:
            print(f"[MODO TESTE] TESTE_LINHAS_ESPECIFICAS={TESTE_LINHAS_ESPECIFICAS}")
        else:
            print(f"[MODO TESTE] TESTE_RELATORIO_INDICE={TESTE_RELATORIO_INDICE}")
            print(f"[MODO TESTE] TESTE_RELATORIO_NOME={TESTE_RELATORIO_NOME or '(não informado)'}")
            print(f"[MODO TESTE] TESTE_RELATORIO_SAVED_VALUE={TESTE_RELATORIO_SAVED_VALUE or '(não informado)'}")
            print(f"[MODO TESTE] TESTE_TIPO_LINHA={TESTE_TIPO_LINHA}")
            print(f"[MODO TESTE] TESTE_LINHA_VALOR={TESTE_LINHA_VALOR}")
        print(f"[MODO TESTE] TESTE_PAGINA_ALVO={TESTE_PAGINA_ALVO}")
    if len(RELATORIOS) <= 1:
